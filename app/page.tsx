...
function TypographyView({ project, update }: { project: DesignSystemProject; update: (recipe: (current: DesignSystemProject) => DesignSystemProject) => void }) {
  ...
  return <div className="view-stack">
    ...
    <Card><SectionHeading level={2} title="Estilos tipográficos" description={`${project.foundations.typography.levels.length} estilos editables. Podés agregar o quitar los que necesite el sistema.`} action={<Button onClick={addLevel}>Agregar estilo</Button>} /><div className="type-level-editor">{project.foundations.typography.levels.map((level) => <article key={level.id}><div className="type-level-fields"><Input label="Nombre" value={level.name} onChange={(event) => editLevel(level.id, { name: event.target.value })} /><Input label="Tamaño" type="number" value={level.size} suffix="px" onChange={(event) => editLevel(level.id, { size: Number(event.target.value) })} /><Input label="Peso" type="number" value={level.weight} onChange={(event) => editLevel(level.id, { weight: Number(event.target.value) })} /><Input label="Interlineado" type="number" step=".05" value={level.lineHeight} onChange={(event) => editLevel(level.id, { lineHeight: Number(event.target.value) })} /><IconButton label={`Eliminar ${level.name}`} disabled={project.foundations.typography.levels.length === 1} onClick={() => update((current) => ({ ...current, foundations: { ...current.foundations, typography: { ...current.foundations.typography, levels: current.foundations.typography.levels.filter((item) => item.id !== level.id) } } }))}><X /></IconButton></div><p style={{ fontFamily: `'${project.foundations.typography.family}', sans-serif`, fontSize: Math.min(level.size, 54), fontWeight: level.weight, lineHeight: level.lineHeight }}>Diseñar con una base coherente.</p></article>)}</div></Card>
  </div>;
}
