...
type DeleteTarget = { kind: "component" | "variant" | "token" | "semantic"; id: string };
function TokensView({ project, update, selected, layer }: { project: DesignSystemProject; update: (recipe: (current: DesignSystemProject) => DesignSystemProject) => void; selected: string; layer: "semantic" | "component" }) {
  ...
  const addSemanticToken = () => update((current) => {
    const count = current.semanticTokens.filter((token) => token.source === "custom" || !requiredSemanticIds.includes(token.id)).length + 1;
    const id = `custom-semantic-${uid()}`;
    return { ...current, semanticTokens: [...current.semanticTokens, { id, name: `custom.role-${count}`, category: "Personalizado", defaultRef: refs[0] || "", themeRefs: {}, platformRefs: {}, description: `Rol semántico personalizado ${count}`, source: "custom" }] };
  });
  ...
  const executeDelete = (leavePending = false) => {
    ...
    update((current) => {
      ...
      if (deleteTarget.kind === "semantic") {
        next.semanticTokens = next.semanticTokens.filter((item) => item.id !== deleteTarget.id);
        if (!leavePending && replacementId) next.componentTokens = next.componentTokens.map((item) => ({ ...item, reference: item.reference === `semantic:${deleteTarget.id}` ? `semantic:${replacementId}` : item.reference, platformRefs: Object.fromEntries(Object.entries(item.platformRefs).map(([key, value]) => [key, value === `semantic:${deleteTarget.id}` ? `semantic:${replacementId}` : value])) }));
      }
...
function ExportPanel({ project, open, onClose, notice }: { project: DesignSystemProject; open: boolean; onClose: () => void; notice: (message: string) => void }) {
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }} title="Configurar exportación" description="Elegí contenido, destino y alcance. Podés crear exportaciones secuenciales sin perder la configuración." className="export-panel-v4"><div className="export-dialog-grid">
    <Card className="export-content-card"><SectionHeading level={2} title="Contenido" /><div className="export-checks">{exportOptions.map((option) => <Checkbox key={option.value} checked={categories.includes(option.value)} onCheckedChange={(checked) => setCategories((current) => checked ? [...current, option.value] : current.filter((item) => item !== option.value))} label={option.label} />)}</div></Card>
    ...
    <Card className="export-scope-card"><SectionHeading level={2} title="Destino y alcance" /><RadioGroup value={format} onValueChange={setFormat} options={[{ value: "json", label: "JSON para Figma y desarrollo" }, { value: "css", label: "Variables CSS" }, { value: "figma-mcp", label: "Paquete para GPT + Figma MCP", meta: "Manifiesto, validación y plan de ejecución en un archivo" }]} /><div className="export-format-stack">
      ...
      <fieldset className="export-platform-scope"><legend>Plataformas activas en Proyecto</legend><p>Marcá cuáles incluir en esta exportación. Esto no cambia la configuración del proyecto.</p>{activePlatforms.map((id) => <Checkbox key={id} checked={platforms.includes(id)} onCheckedChange={(checked) => togglePlatform(id, checked)} label={project.platforms[id].name} />)}</fieldset>
      {inactivePlatforms.length ? <fieldset className="export-platform-scope inactive"><legend>Plataformas desactivadas en Proyecto</legend><p>Son opcionales para esta salida y permanecen desactivadas en el proyecto.</p>{inactivePlatforms.map((id) => <div key={id}><Checkbox checked={platforms.includes(id)} onCheckedChange={(checked) => togglePlatform(id, checked)} label={project.platforms[id].name} /><small>Desactivada en Proyecto · conserva configuración · {Object.keys(project.platforms[id].overrides).length || Object.keys(project.platforms[id].scaleOverrides).length ? "Con overrides" : "Hereda de Mobile"}</small></div>)}</fieldset> : null}
    ...
    <Card className="export-secondary-card"><SectionHeading level={2} title="Otras salidas" description="El archivo editable conserva siempre todas las plataformas y el sitio HTML documenta el alcance elegido del proyecto." /><div className="export-actions-v4"><Button onClick={() => downloadText(projectFilename(project, ".dslab.json"), JSON.stringify(project, null, 2))}>Descargar proyecto editable</Button><Button onClick={() => downloadText(projectFilename(project, "-docs.html"), buildDocumentation(project), "text/html")}>Descargar documentación HTML</Button></div></Card></div>
  </Dialog>;
}
...
export default function Home() {
  ...
  return <div suppressHydrationWarning className={`lab-v4 theme-${theme}`} data-lab-theme={theme}>
