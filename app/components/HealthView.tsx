...
function Scenario({ project, platform, theme }: { project: DesignSystemProject; platform: PlatformId; theme: string }) {
  ...
  const titleId = `scenario-${platform}-title`;
  return <article className={`health-scenario scenario-${platform}`} aria-labelledby={titleId}><header><div><span>{project.meta.brandMark}</span><b>{project.meta.name}</b></div><div className="scenario-nav" aria-hidden="true">Inicio&nbsp;&nbsp; Equipo&nbsp;&nbsp; Ajustes</div></header><div className="scenario-body"><div className="scenario-title"><div><small>Equipo de producto</small><h3 id={titleId}>Revisión del sistema</h3><p>Comprobá el formulario y la tabla antes de publicar.</p></div><button>Crear revisión</button></div><div className="scenario-grid"><section aria-labelledby={`${titleId}-form`}><h4 id={`${titleId}-form`}>Nueva revisión</h4><label>Nombre<input defaultValue="Auditoría de contraste" /></label><label>Responsable<button className="scenario-select" role="combobox" aria-expanded="false" aria-controls={`${platform}-responsible-options`}>Equipo de diseño <ChevronDownIcon /></button><span hidden role="listbox" id={`${platform}-responsible-options`}><span role="option" aria-selected="true">Equipo de diseño</span></span></label><div className="scenario-feedback"><ProjectAlertPreview state="Warning">Hay decisiones heredadas que requieren revisión.</ProjectAlertPreview></div><button>Guardar cambios</button></section><section aria-labelledby={`${titleId}-activity`}><h4 id={`${titleId}-activity`}>Actividad reciente</h4><div className="scenario-table"><div><b>Foundation</b><b>Estado</b></div><div><span>Paleta primaria</span><em>Lista</em></div><div><span>Grilla responsiva</span><em>Pendiente</em></div><div><span>Tokens de foco</span><em>Lista</em></div></div></section></div></div>{!snapshot.ready ? <div className="scenario-pending">Configuración pendiente: {snapshot.missing.length} referencias</div> : null}</article>;
}
