...
import { componentTokenPath, DesignSystemProject, PlatformId, platformOrder } from "../lib/model";
import { resolveProjectTokens } from "../lib/token-resolver";
import { Alert, Badge, Button, Card, SectionHeading, Table } from "./ui/LabUI";

type Props = { project: DesignSystemProject; onOpenTokens: (token: string) => void; onOpenCatalog: (component?: string) => void; onOpenScenarios: (component?: string) => void; onOpenProject: () => void; onOpenFinding: (finding: HealthFinding) => void };
...
export function HealthView({ project, onOpenTokens, onOpenCatalog, onOpenScenarios, onOpenProject, onOpenFinding }: Props) {
  ...
  const coverage = useMemo(() => project.themes.flatMap((mode) => enabledPlatforms.map((platform) => {
    ...
    const missingComponents = catalogRegistry.filter((entry) => entry.componentTokens.some((path) => !project.componentTokens.some((token) => componentTokenPath(project, token) === path)));
    const errors = snapshot.ready ? 0 : Math.max(1, snapshot.missing.length);
    const warnings = project.platforms[platform].proposalPending ? 1 : 0;
  ...
  return <div className="health-v4">
    ...
    <Card><SectionHeading level={2} title="Cobertura visual e inclusiones" description="Estos datos son informativos: no penalizan componentes exportables sin renderer ni plataformas desactivadas." /><div className="health-information-metrics"><button type="button" onClick={() => onOpenCatalog()}><b>{health.metrics.componentsWithPreview}</b><span>Componentes con preview</span></button><button type="button" onClick={() => onOpenCatalog()}><b>{health.metrics.variantsEvaluated}</b><span>Variantes evaluadas</span></button><button type="button" onClick={() => onOpenTokens(project.components.find((component) => !component.rendererKey)?.key || "")}><b>{health.metrics.customWithoutRenderer}</b><span>Componentes sin renderer</span></button><button type="button" onClick={() => onOpenTokens(project.componentTokens.find((token) => !project.components.find((component) => component.id === token.componentId)?.rendererKey)?.key || "")}><b>{health.metrics.tokensWithoutVisual}</b><span>Tokens sin representación</span></button><button type="button" onClick={onOpenProject}><b>{health.metrics.inactivePlatforms}</b><span>Plataformas inactivas</span></button></div></Card>
