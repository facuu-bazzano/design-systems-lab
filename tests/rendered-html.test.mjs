...
test("keeps v5 capabilities in one serializable project model", async () => {
  ...
  assert.match(model, /schemaVersion: 5/);
  assert.match(model, /semanticTokens: SemanticToken\[\]/);
  assert.match(model, /componentTokens: ComponentToken\[\]/);
  assert.match(model, /components: ProjectComponentDefinition\[\]/);
  assert.match(model, /componentVariants: ComponentVariant\[\]/);
